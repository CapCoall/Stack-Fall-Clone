...
public class Player : MonoBehaviour
{
    ...
    void Update()
    {
        if (playerState == PlayerState.Playing)
        {
            ...
            if (currentTime >= 0.15f || invisibleFill.color == Color.red)
            ...
            if (currentTime >= 1)
            {
                currentTime = 1;
                invincible = true;
                invisibleFill.color = Color.gray;
            }
            else if (currentTime <= 0)
            {
                currentTime = 0;
                invincible = false;
                invisibleFill.color = Color.white;
            }
            if (invisibleObj.activeInHierarchy)
                invisibleFill.fillAmount = currentTime /1;
        }
        ...
        if (playerState == PlayerState.Finish)
        {
            if (Input.GetMouseButtonDown(0))
            {
                Debug.Log("game over ekranýnda mouseye bastýn");
            }
        }
    }
    ...
    private void OnCollisionEnter(Collision target)
    {
        ...
        if (target.gameObject.tag == "Finish" && playerState == PlayerState.Playing)
        {
            ...
            win.transform.localPosition = Vector3.up * 1.3f;
            win.transform.eulerAngles = Vector3.zero;
            FindObjectOfType<GameUI>().NextLevelUI();
        }
    }
